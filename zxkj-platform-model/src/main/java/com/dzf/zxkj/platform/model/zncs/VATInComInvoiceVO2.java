 ...
 *
 */
public class VATInComInvoiceVO2 extends SuperVO implements IGlobalPZVO {
	...
	private String pk_basecategory; //基础类别主键
	...
	private String busisztypecode;//业务类型结算方式（暂时没用）
...
//	private DZFBoolean isic;//是否生成库存
	...
	private DZFDouble zje;//总金额（展示用，不存库）
	private DZFDouble zse;//总税额（展示用，不存库）
	private DZFDouble zjshj;//总价税合计（展示用，不存库）
	...
	private String xfdh;//销方电话（认证平台取机动车票用）
	private String xfyhzh;//销方银行账号（认证平台取机动车票用）
	...
	private String fplx;//发票类型  导入认证平台数据     机动车发票   增值税专票 普票。。。。
...
//	-------------一键取票参数-----------------------
	...
	public String getRzssq() {
		return rzssq;
	}
	public void setRzssq(String rzssq) {
		this.rzssq = rzssq;
	}
	...
	public DZFDouble getZje() {
		return zje;
	}
	public void setZje(DZFDouble zje) {
		this.zje = zje;
	}
	...
	public DZFDouble getZjshj() {
		return zjshj;
	}
	public void setZjshj(DZFDouble zjshj) {
		this.zjshj = zjshj;
	}
