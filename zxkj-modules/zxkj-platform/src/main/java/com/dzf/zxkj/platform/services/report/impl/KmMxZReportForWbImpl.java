...
import com.dzf.zxkj.platform.services.report.IKmMxZReportForWb;
...
public class KmMxZReportForWbImpl implements IKmMxZReportForWb {
	...
	private IPersonalSetService gl_gxhszserv =null;
	
	@Autowired
	private IAuxiliaryAccountService accountservice;
    ...
    private Map<String, AuxiliaryAccountBVO> getFzXm(String pk_corp) {
    	...
    	SQLParameter sp = new SQLParameter();
    	sp.addParam(pk_corp);
    	AuxiliaryAccountBVO[] bvos = accountservice.queryAllB(pk_corp);
	...
	private Map<String, YntCpaccountVO> getKM(String pk_corp, String kmwhere)
		...
		/** 个性化设置vo */
		GxhszVO myselfset = gl_gxhszserv.query(pk_corp);
		/** 科目现在方式，默认显示本级 */
	...
	public IPersonalSetService getGl_gxhszserv() {
		return gl_gxhszserv;
	}

	@Autowired
	public void setGl_gxhszserv(IPersonalSetService gl_gxhszserv) {
		this.gl_gxhszserv = gl_gxhszserv;
	}
