					...
					if (obt != null) {
						...
						for (Class str : IBackAndRestore.classes) {
							...
							if (tempvo.getTableName().equals(key)) {
								...
								tempvo = JsonUtils.deserialize(obt.toString(), tempvo.getClass());
//								for(String name:names){
//									if(tempvo.getAttributeValue(name)!=null 
//											&& tempvo.getAttributeValue(name) instanceof String){
//										if(tempvo.getAttributeValue(name).toString().startsWith("003quy")){
//											tempvo.setAttributeValue(name, "003rLU"+tempvo.getAttributeValue(name).toString().substring(6));
//										}
//									}
//								}
////								tempvo.setPrimaryKey("003rLU"+tempvo.getPrimaryKey().substring(6));
//								tempvo.setAttributeValue("pk_corp", "003rLU");//更改公司的值
								if (tempvo != null) {
...
//								sp.addParam(cpvo.getVstateuname());//国税用户名
				...
				//CorpCache.getInstance().remove(pk_corp);
				//AccountCache.getInstance().remove(pk_corp);
			}
		} catch (Exception e) {
			throw new WiseRunException(e);
		}
