...
public class ExcelReport<T> {
	...
	public void exportExcel(Collection<T> dataset, OutputStream out, String gs, String qj,	IUserService userServiceImpl) {
	...
	public byte[] exportExcel(String sheetName, String[] headers, String[] fields, Collection<T> dataset, String gs,
			String qj, OutputStream out,	IUserService userServiceImpl) {
	...
	public byte[] exportExcel(String sheetName, String[] headers, String[] fields, Collection<T> dataset, String gs,
			String qj, OutputStream out, String f,	IUserService userServiceImpl) {
		flag = f;
		return exportExcel(sheetName, headers, fields, dataset, out, "yyyy-MM-dd", gs, qj,userServiceImpl);
	}

	public void exportExcel(String[] headers, String[] fields, Collection<T> dataset, String gs, String qj,
			OutputStream out, String pattern,	IUserService userServiceImpl) {
...
//		}
	...
	public byte[] exportExcel(String title, String[] headers, String[] fields, Collection<T> dataset, OutputStream out,
			String pattern, String gs, String qj,IUserService userServiceImpl) {
		// 声明一个工作薄
...
//					IUserService userServiceImpl = SpringUtils.getBean("userServiceImpl");
						UserVO user = userServiceImpl.queryUserJmVOByID((String) value);// UserCache.getInstance().get((String) value, null);
						if (user != null) {
							HSSFCell cell4ss = row_names.createCell(((headers.length - 1) / 2) + 2);
							cell4ss.setCellStyle(style4);
		...
		if (decimalCols != null && Arrays.binarySearch(decimalCols, field) >= 0) { // 右对齐，精度为2
			if (field.equals("ybjf") || field.equals("ybdf") || field.equals("ybye")
				...
				if(!StringUtil.isEmpty(pk_corp)){
					Integer jd =  ReportUtil.getHlJd(pk_corp);
					key = "right_"+jd;
				}else{
					key = "right_4";
				}
