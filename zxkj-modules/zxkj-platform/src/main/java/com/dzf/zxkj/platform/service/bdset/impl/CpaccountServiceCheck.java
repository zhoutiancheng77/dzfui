...
//		}
	 ...
	 * 新增科目前校验
	 */
	public void preCheckAdd(YntCpaccountVO vo, CorpVO corpvo)throws BusinessException{
		
		YntCpaccountVO parentvo = queryYntvoByid(vo.getPk_corp_account());
	 ...
	 * @return
	 */
	public boolean checkIsQuote(YntCpaccountVO vo) throws BusinessException{
		...
		boolean ischd = false;
		if(parentvo != null){
			ischd = isHasChilden(vo.getPk_corp(),parentvo.getAccountcode());
		}
		if(!ischd){
			boolean ret = checkIsQC(vo.getPk_corp(),parentvo);
			if(ret){
				return ret;
			}
			ret = checkIsPzmb(vo.getPk_corp(),parentvo);
			if(ret){
				return ret;
			}
			ret = checkIsPz(vo.getPk_corp(),parentvo);
			if(ret){
				return ret;
			}
			ret = checkIsGzb(vo.getPk_corp(),parentvo);
			if(ret){
				return ret;
			}
			//  校验是否是新库存模式
			...
			if (corpvo.getIbuildicstyle() != null && corpvo.getIbuildicstyle() == 1) {
				ret =checkIsIc(vo.getPk_corp(), parentvo);
				if(ret){
					return ret;
				}
			}
	 ...
	 * 校验----是否被库存模块引用
	 */
	private boolean checkIsIc(String pk_corp, YntCpaccountVO parentVO) throws BusinessException {
...
//		sqlp1.addParam(pk_corp);
//		//入账科目设置
//		ret = isExists(pk_corp, "select 1 from bd_invaccset where ( cg_yjjxskm= ? or cg_yfzkkm =? or cg_xjfkkm =? or xs_xjskkm =?"
//				+ " or xs_yszkkm =? or xs_yysrkm =? or xs_yjxxskm =? or ll_clcbkm =? or ll_yclkm =? "
//				+ " or vdef1 =? or vdef2 =? or vdef3 =? or vdef4 =? or vdef5 =? or vdef6 =?) and pk_corp = ?  and nvl(dr,0)=0 ",
//				sqlp1);
//		if (ret) {
//			return ret;
//		}
	 ...
	 * 校验----科目是否被凭证模板引用
	 */
	private boolean checkIsPzmb(String pk_corp,YntCpaccountVO parentVO)throws BusinessException{
		//成本模板
	 ...
	 * 校验----科目是否被凭证引用
	 */
	public boolean checkIsPz(String pk_corp,YntCpaccountVO parentVO)throws BusinessException{
		SQLParameter sqlp=new SQLParameter();
	...
	public boolean checkIsGzb(String pk_corp,YntCpaccountVO parentVO)throws BusinessException{
		SQLParameter sqlp=new SQLParameter();
		sqlp.addParam(pk_corp);
		sqlp.addParam(parentVO.getPrimaryKey());
	...
	public boolean checkQCYE(YntCpaccountVO vo) {
		//是否被期初引用
		SQLParameter sqlp =new SQLParameter();
		sqlp.addParam(vo.getAccountcode());
		sqlp.addParam(vo.getPk_corp());
		String sql = " select * from ynt_qcye where vcode = ? and pk_corp = ? and nvl(dr,0)=0 and (nvl(bnqcnum,0) !=0  or nvl(bnfsnum,0) !=0  or nvl(bndffsnum,0) !=0  ) ";
		List<QcYeVO> list = (List<QcYeVO>) singleObjectBO.executeQuery(sql, sqlp, new BeanListProcessor(QcYeVO.class));
		//是否被凭证引用
		String sql2 = " select * from ynt_tzpz_b where vcode= ? and pk_corp = ?  and nvl(dr,0)=0  ";
		List<TzpzBVO> list2 = (List<TzpzBVO>) singleObjectBO.executeQuery(sql2, sqlp, new BeanListProcessor(TzpzBVO.class));
	...
	public boolean checkFzhsQCYE(YntCpaccountVO vo) {
		SQLParameter sqlp =new SQLParameter();
		sqlp.addParam(vo.getPk_corp_account());
		sqlp.addParam(vo.getPk_corp());
		//是否被期初辅助核算引用
		String sql = " select * from ynt_fzhsqc where pk_accsubj = ? and pk_corp = ? and thismonthqc is not null and thismonthqc <> 0 and nvl(dr,0)=0 ";
		List<FzhsqcVO> list = (List<FzhsqcVO>) singleObjectBO.executeQuery(sql, sqlp, new BeanListProcessor(FzhsqcVO.class));
		//是否被期初引用
		String sql1 = " select * from ynt_qcye where pk_accsubj = ? and pk_corp = ? and thismonthqc is not null and thismonthqc <> 0 and nvl(dr,0)=0 ";
		List<QcYeVO> list1 = (List<QcYeVO>) singleObjectBO.executeQuery(sql1, sqlp, new BeanListProcessor(QcYeVO.class));
		//是否被凭证引用
		//String sql2 = " select * from ynt_tzpz_b where pk_accsubj = ? and pk_corp = ?  and nvl(dr,0)=0  ";
		//List<TzpzBVO> list2 = (List<TzpzBVO>) singleObjectBO.executeQuery(sql2, sqlp, new BeanListProcessor(TzpzBVO.class));
	...
	public boolean checkParentVerification(YntCpaccountVO vo) {
		...
		if (vo.getAccountcode().length() == 4)
			return isVerify;
		SQLParameter sqlp =new SQLParameter();
		sqlp.addParam(vo.getAccountcode().substring(0, 4) + "%");
		sqlp.addParam(vo.getPk_corp());
		String condition = " accountcode like ? and pk_corp = ? and nvl(dr,0)=0 and isverification = 'Y' ";
		YntCpaccountVO[] rs = (YntCpaccountVO[]) singleObjectBO.queryByCondition(YntCpaccountVO.class, condition, sqlp);
		if (rs != null) {
			for (YntCpaccountVO account : rs) {
				if (account.getAccountcode().length() < vo.getAccountcode().length()) {
					isVerify = true;
					break;
				}
			}
