...
//	private String lastRequestTime;//上次请求时间
	...
	public PiaoTongJinXiang(String taxNum,
			String f2,
...
//		this.lastRequestTime = lastRequestTime;
    ...
    public List<PiaoTongJinXiangHVO> getVOs(String token){
    	...
    	//2、获取概要信息
//    	i = 2;
    	List<PiaoTongJinXiangInvoiceVO> gyList = getInvoiceList(token);
...
//    	i = 3;
    	...
    	for(PiaoTongJinXiangInvoiceVO vo : gyList){
    		if(!("01".equals(vo.getInvoiceTicketType())
    				...
    				|| "11".equals(vo.getInvoiceTicketType())))//暂时先如此处理
    			continue;
...
//    			|| datavo.getInvoiceContent() == null
    	...
    	detailvo.setSalesName(datavo.getSalesName());
    	...
    	detailvo.setSalesTaxNum(datavo.getSalesTaxNum());
     ...
     * 获取最新采集内容 概要信息
     */
    public List<PiaoTongJinXiangInvoiceVO> getInvoiceList(String token){
//    	String url = ptjxurl + "/invoice/getInvoiceCollection.pt";
    	...
    	paramMap.put("token", getQuotes(token));
    	paramMap.put("taxNum", getQuotes(taxNum));
    	paramMap.put("year", getQuotes(period.substring(0, 4)));
    	paramMap.put("month", getQuotes(period.substring(5,7)));
