...
//		DZFDouble currentIncomeMny = lrbInfo[0];
        ...
        if (isFixed) {
            ...
            if (isIndividual) {
                calTax = getIndividualIncomeTaxFixedRate(period, yearIncomeMny, paidTax, fixedRate);
            } else {
                //优惠政策是否变动
                String yhzc = buildYhzc(taxInfo.getVyhzc(), period, pk_corp);
                calTax = getCorporateIncomeTaxFixedRate(period,
                        yearIncomeMny, paidTax, fixedRate, yhzc);
            }
        } else {
            if (isIndividual) {
                calTax = getIndividualIncomeTaxActual(pk_corp, taxInfo, period, yearIncomeMny, lossmny, paidTax);
            } else {
                //优惠政策是否变动
                String yhzc = buildYhzc(taxInfo.getVyhzc(), period, pk_corp);
                calTax = getCorporateIncomeTaxActual(period,
    ...
    //优惠政策是否要调整
    public static String buildYhzc(String yhzc, String period, String pk_corp) {
        ...
        IIncomeWarningService gl_incomewarningerv = (IIncomeWarningService) SpringUtils
                .getBean("gl_incomewarningerv");
        DZFDouble value = gl_incomewarningerv.getSpecFsValue(periods[0], periods[1], pk_corp, vo);
        value = value == null ? DZFDouble.ZERO_DBL : value;
        //平均【资产总额】是否<=5000 万元：取计提当期该账套“资产负债表”的
        // 【资产合计】表项的（季初资产总额+季末资产总额）/2 是否<=5000 万元
        if (value.doubleValue() > 50000000) {
            yhzc = "9";//不享受优惠
        }
    ...
    // 企业所得税-查账征收
    private DZFDouble getCorporateIncomeTaxActual(String period,
        ...
        DZFDouble calTax = calculateCorporateIncomeTax(taxableIncome, paidTax, period, yhzc);
        return calTax;
    }

    // 企业所得税-核定应税所得率征收
    private DZFDouble getCorporateIncomeTaxFixedRate(String period, DZFDouble yearIncomeMny,
    ...
    private DZFDouble getIndividualIncomeTaxActual(String pk_corp, CorpTaxVo taxInfo,
                                                   String period, DZFDouble yearIncomeMny,
        ...
        DZFDouble calTax = SafeCompute.multiply(taxableIncome, rate);
    ...
    // 个人所得税-核定应税所得率征收
    private DZFDouble getIndividualIncomeTaxFixedRate(String period, DZFDouble yearIncomeMny,
