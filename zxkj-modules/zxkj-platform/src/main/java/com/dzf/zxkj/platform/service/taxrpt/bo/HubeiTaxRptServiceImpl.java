...
@Service("taxRptservice_hubei")
public class HubeiTaxRptServiceImpl extends DefaultTaxRptServiceImpl {
	@Override
	public String checkReportData(Map mapJson, CorpVO corpvo,
		...
		if (reportvo.getSb_zlbh().equals(TaxRptConst.SB_ZLBH10412)){
			errmsg = checkForSB_ZLBH10412_hb(mapJson, corpvo, reportvo, hmRptDetail, sbo);
		}
		
		return errmsg;
	}
	
	private String checkForSB_ZLBH10412_hb(Map mapJson, CorpVO corpvo, TaxReportVO reportvo,
		...
		if (listReportName.contains(rpt1)) {
			...
			if ("一般企业".equals(qylx) || "跨地区经营汇总纳税企业总机构".equals(qylx)) {
				...
				if (getDzfDouble(spreadtool.getCellValue(mapJson, rpt1, 33, 3)).doubleValue() <= 0) {
					errmsg += "企业类型为“一般企业”或“跨地区经营汇总纳税企业总机构”的纳税人必须填写期末从业人数，且人数必须大于0且为整数<br>";
				}
