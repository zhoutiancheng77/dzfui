...
public class Formula {
	...
	private Vector<Integer> paras = new Vector<Integer>();// 用来存放变量参数
...
//				String.valueOf(value);
	...
	private void symbolParse(String str) {
		...
		if (sym[0] == -1 && sym[1] == -1 && sym[2] == -1 & sym[3] == -1) {
			if (containsLetter(str)&&!"E".equals(str.substring(str.length()-2, str.length()-1))) {
...
//
//		Formula test1 = new Formula("1.221*-1");// 测试常规的运算公式
//
//		paras.add(5);// 为公式里的变量存数据
//		paras.add(3);// 为公式里的变量存数据
//		paras.add(1);
//		Formula test2 = new Formula("aa-bb+(cc*2)+10.1");// 把两个变量分别替换成4和3再进行计算
//
//		System.out.println(test1.getResult());
//		System.out.println(test2.getResult());
//		String [] ss ="1.21+2*(3+(4/2))".split("\\+|-|\\*|/|\\(|\\)");
