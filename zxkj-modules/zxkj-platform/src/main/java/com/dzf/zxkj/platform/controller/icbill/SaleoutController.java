...
public class SaleoutController{
	...
	public ReturnData saveSale(@RequestParam Map<String, String> param) {
		...
		try {
			...
			IntradeoutVO[] bodyvos =JsonUtils.convertValue(body,IntradeoutVO[].class); // 表体
			// 赋默认值
...
//						ISysConstants.SYS_2);
	...
	public ReturnData delSale(@RequestParam Map<String, String> param) {
		...
		try {
			...
			body = body.replace("}{", "},{");
			body = "[" + body + "]";
			bodyvos = JsonUtils.convertValue(param, IntradeHVO[].class);
...
//					ISysConstants.SYS_2);
	...
	public ReturnData togl(@RequestParam Map<String, String> param) {
		...
		try {
			...
			body = body.replace("}{", "},{");
			body = "[" + body + "]";
			IntradeHVO[] bodyvos =bodyvos = JsonUtils.convertValue(param, IntradeHVO[].class);
...
//		writeLogRecord(LogRecordEnum.OPE_KJ_IC_BUSI.getValue(), "出库单转总账", ISysConstants.SYS_2);
	...
	public ReturnData toTotalGL(@RequestParam Map<String, String> param) {
		...
		try {
			...
			body = body.replace("}{", "},{");
			body = "[" + body + "]";
            IntradeHVO[]  bodyvos = JsonUtils.convertValue(param, IntradeHVO[].class);
			if (bodyvos == null || bodyvos.length == 0) {
				throw new BusinessException("数据为空,转总账失败!");
			}
			String zy = "销售商品";
