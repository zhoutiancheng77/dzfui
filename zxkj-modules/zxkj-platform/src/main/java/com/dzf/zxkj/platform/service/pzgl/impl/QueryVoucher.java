...
public class QueryVoucher {
    ...
    public QueryPageVO queryVoucherPaged(VoucherParamVO paramvo, boolean useTemplateTable)
...
//        wheresql.append(" where ynt_tzpz_h.pk_corp = ? ");
        ...
        String querySql;
        if (useTemplateTable) {
            querySql = prepareVoucherToTempTable(join, where, orderField, sp, paramvo.getPage(), paramvo.getRows());
        } else {
            sqlParam = sp;
            querySql = getMainCauseSql(join, where, orderField, paramvo.getPage(), paramvo.getRows());
        }
        ...
        headSql.append("select ynt_tzpz_h.*,  zd.user_name as zd_user,sh.user_name as sh_user, ")
                ...
                .append(" where ynt_tzpz_h.rowid in (")
                .append(querySql)
                .append(") order by ")
                .append(orderField);
        ...
        List<TzpzHVO> hvo = (List<TzpzHVO>) singleObjectBO
                .executeQuery(headSql.toString(),
                        sqlParam, new BeanListProcessor(TzpzHVO.class));
        ...
        if (hvo != null && hvo.size() > 0) {
            ...
            sql.append("   from ynt_tzpz_h where rowid in (");
            sql.append(querySql);
            sql.append(") ) th ");
    ...
    private String prepareVoucherToTempTable(String join, String where, String orderFields,
                                             SQLParameter sp, int pageNo, int pageSize) {
        String insertSql = "insert into dzf_tmp_insql(a)"
                + getMainCauseSql(join, where, orderFields, pageNo, pageSize);
        singleObjectBO.executeUpdate(insertSql, sp);
        return "select a from dzf_tmp_insql";
    }

    // 主表条件语句
    private  String getMainCauseSql(String join, String where, String orderFields,
                                    int pageNo, int pageSize) {
        StringBuilder pagedSql = new StringBuilder();
        pagedSql.append("select rid from (select rownum rn, tem.rid from (")
                .append(" select ");
    ...
    public TzpzHVO queryVoucherById(String id) throws BusinessException {
        if (id == null) {
            throw new BusinessException("传入查询条件为空！");
        }
