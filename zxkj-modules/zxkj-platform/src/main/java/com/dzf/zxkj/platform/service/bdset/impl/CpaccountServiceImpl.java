...
public class CpaccountServiceImpl implements ICpaccountService {
    ...
    private YntCpaccountVO insertByCorpAccountVO(String pk_corp,
        ...
        check.checkAdd(vo);
        YntCpaccountVO parentVO = check.getParentVOByID(vo.getPk_corp(), vo.getAccountcode());

        if (parentVO == null)
            throw new BusinessException("当前新增科目，上级科目不存在！");

        if (check.checkParentVerification(vo.getPk_corp(), vo.getAccountcode())) {
        ...
        boolean isQuote = checkIsQuote(vo.getPk_corp(), vo.getAccountcode());
        // 科目全称
        if (parentVO != null) {
...
//		checkFsye(svo,corpvo); 移到action中
        ...
        if ((svo.getIsverification() == null || !svo.getIsverification().booleanValue())
                && check.checkParentVerification(svo.getPk_corp(), svo.getAccountcode())) {
    ...
    public void deleteInfovo(YntCpaccountVO bean) throws DZFWarpException {
        ...
        YntCpaccountVO parentvo = check.getParentVOByID(svo.getPk_corp(), svo.getAccountcode());
    ...
    public void updateSeal(YntCpaccountVO vo) throws DZFWarpException {
        ...
        queryParamVO.setBegindate1(nowDate);
        queryParamVO.setEnddate(nowDate);
        queryParamVO.setCjq(1);
        queryParamVO.setCjz(8);
        queryParamVO.setKms_first(vo.getAccountcode());
        queryParamVO.setKms_last(vo.getAccountcode());
        queryParamVO.setPk_corp(vo.getPk_corp());
        queryParamVO.setIshasjz(DZFBoolean.FALSE);
        queryParamVO.setIsLevel(DZFBoolean.FALSE);
        queryParamVO.setXsyljfs(DZFBoolean.FALSE);
        queryParamVO.setXswyewfs(DZFBoolean.FALSE);
        FseJyeVO[] fseVos = (FseJyeVO[]) zxkjReportService.getFsJyeVOs(queryParamVO, 1);
        if (fseVos != null && fseVos.length > 0) {
            FseJyeVO fse = fseVos[0];
            DZFDouble temp = DZFDouble.ZERO_DBL;
            if ("借".equals(fse.getFx())) {
                temp = fse.getQmjf();
            } else {
    ...
    @Override
    public boolean checkIsQuote(String corpId, String subjectCode) throws DZFWarpException {
        CpaccountServiceCheck check = new CpaccountServiceCheck(singleObjectBO);
        return check.checkIsQuote(corpId, subjectCode);
    }

    @Override
    public boolean checkBeginDataRef(String corpId, String subjectId) throws DZFWarpException {
        CpaccountServiceCheck check = new CpaccountServiceCheck(singleObjectBO);
        return check.checkBeginDataRef(corpId, subjectId);
    }

    @Override
    public boolean checkIsPzRef(String corpId, String subjectId) throws DZFWarpException {
        CpaccountServiceCheck check = new CpaccountServiceCheck(singleObjectBO);
        return check.checkIsPz(corpId, subjectId);
    }
    ...
    public Set<String> getCurrencyRefSet(String corpId, String subjectId) throws DZFWarpException {
        ...
        List<String> list1 = (List<String>) singleObjectBO.executeQuery(sql1, sqlp, new ColumnListProcessor());
        List<String> list2 = (List<String>) singleObjectBO.executeQuery(sql2, sqlp, new ColumnListProcessor());
        List<String> list3 = (List<String>) singleObjectBO.executeQuery(sql3, sqlp, new ColumnListProcessor());
    ...
    public void saveSyncTdAccount(CorpVO cvo) throws DZFWarpException {
        ...
        for (int i = 0, size = list.size(); i < size; i++) {
            ...
            // 取上级vo
            YntCpaccountVO pvo = check.getParentVOByID(vo.getPk_corp(), vo.getAccountcode());
            insertVOs.add(vo);
            if (pvo != null) {
    ...
    public boolean checkParentVerification(String corpId, String subjectCode)
            throws DZFWarpException {
        CpaccountServiceCheck check = new CpaccountServiceCheck(singleObjectBO);
        return check.checkParentVerification(corpId, subjectCode);
    }
    ...
    @Override
    public Map<String, List<YntCpaccountVO>> queryAccountVO(String userid, String pk_corp,
                                                            boolean excludeSealed) throws DZFWarpException {
        if (StringUtil.isEmpty(pk_corp))
            return null;
        SQLParameter sp = new SQLParameter();
        sp.addParam(pk_corp);
...
//			}
    ...
    public YntCpaccountVO[] queryCpAccountVOs(String pk_corp, String accountcode) {
        SQLParameter sp = new SQLParameter();
        sp.addParam(accountcode + "%");
        sp.addParam(pk_corp);
        sp.addParam("Y");
    ...
    @Override
    public void checkBeforeAdd(String subjectId, CorpVO corpvo) {
        CpaccountServiceCheck check = new CpaccountServiceCheck(singleObjectBO);
        check.preCheckAdd(subjectId, corpvo);
    }
    ...
    public String getNewSubCode(YntCpaccountVO vo, CorpVO corpvo) {
        ...
        if (level != null && level < levelNum.length) {
            ...
            List<String> subCodes = (List<String>) singleObjectBO.executeQuery(sql, sp, new ColumnListProcessor());
            int subCode = 0;
            if (subCodes.size() == 0) {
                // 没有下级
                subCode = 1;
            } else {
                int maxAllow = (int) Math.pow(10, subLevelLen) - 1;
                if (subCodes.size() == maxAllow) {
                    // 编码已占满
                    subCode = 0;
                } else {
                    subCodes.sort(String::compareTo);
                    int parentCodeLen = parentCode.length();
                    int maxCode = Integer.valueOf(subCodes.get(subCodes.size() - 1).substring(parentCodeLen));
                    if (maxCode == maxAllow) {
                        // 已到最大编码，找编码补位
                        List<Integer> codeNums = subCodes.stream()
                                .map(code -> Integer.valueOf(code.substring(parentCodeLen)))
                                .collect(Collectors.toList());
                        int endIndex = codeNums.size() - 1;
                        Integer discontinuityCode = codeNums.get(endIndex);
                        for (int i = 0; i < endIndex; i++) {
                            Integer codeNum = codeNums.get(i);
                            Integer nextNum = codeNums.get(i + 1);
                            if (nextNum != codeNum + 1) {
                                discontinuityCode = codeNum;
                                break;
                            }
                        }
                        subCode = discontinuityCode + 1;
                    } else {
                        subCode = maxCode + 1;
                    }
                }
            }
            newCode += String.format("%0" + subLevelLen + "d", subCode);
        }
        return newCode;
    }
