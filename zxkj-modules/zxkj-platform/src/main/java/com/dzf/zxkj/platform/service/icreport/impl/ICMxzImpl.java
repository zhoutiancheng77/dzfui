...
@Service("ic_rep_mxzserv")
@Slf4j
public class ICMxzImpl implements IICMxz {
	...
	private List<IcDetailVO> queryIcDetailsMode(QueryParamVO paramvo, CorpVO corpvo) {
		...
		sf.append(" ry.pk_subject pk_accsubj,nt.accountcode kmbm,nt.accountname km,fy.name spfl, fy.code spflcode,fy.pk_invclassify spflid,");
	...
	private List<IcDetailVO> queryIcDetailsMode2(QueryParamVO paramvo, CorpVO corpvo) {
		...
		if (isChargedept) {
			sf.append("case  when nvl(i.ncost1,0)<>0 then i.ncost1 else (CASE nvl(h1.fp_style,2) WHEN 2 THEN  i.nymny ELSE i.ntotaltaxmny END) end nymny,");
			sf.append("case  when nvl(i.ncost1,0)<>0 then i.ncost1 else (CASE nvl(h1.fp_style,2) WHEN 2 THEN  i.nymny ELSE i.ntotaltaxmny END) end ncost,");
		} else {
	...
	private Map<String, IcQcVO> getICQcMx(QueryParamVO paramVo) {
		...
		// StringBuffer sf = new StringBuffer();
		// sf.append(" select y.*,ry.pk_subject pk_accsubj,nt.accountcode
		// kmbm,nt.accountname km,fy.name spfl, ");
		// sf.append(" ry.invspec spgg, ry.invtype spxh, re.name jldw , ry.code
		// spbm, ry.name spmc ");
		// sf.append(" from ynt_fzhsqc y ");
		// sf.append(" left join ynt_inventory ry on ry.pk_inventory = y.fzhsx6
		// ");
		// sf.append(" left join ynt_invclassify fy on fy.pk_invclassify =
		// ry.pk_invclassify ");
		// sf.append(" left join ynt_measure re on re.pk_measure = ry.pk_measure
		// ");
		...
		// sf.append(" and y.fzhsx6 is not null ");
		// sf.append(" and pk_accsubj in (select pk_corp_account ");
		// sf.append(" from ynt_cpaccount cp ");
		// sf.append(" where cp.pk_corp = ? ");
		// sf.append(" and nvl(cp.dr, 0) = 0 ");
		// sf.append(" and isnum = 'Y' ");
		// sf.append(" and isleaf = 'Y') ");
		// if(!StringUtil.isEmptyWithTrim(paramVo.getPk_inventory())){
		// String[] spris = paramVo.getPk_inventory().split(",");
		// String wheInv = SqlUtil.buildSqlConditionForIn(spris);
		// sf.append(" and y.fzhsx6 in ( ");
		// sf.append(wheInv);
		// sf.append(" ) ");
		// }
		StringBuffer sf = new StringBuffer();
	...
	private Map<String, IcDetailVO> createResult(Map<String, IcQcVO> qcMap, List<Map<String, List<IcDetailVO>>> fsMap,
		...
		if (keySet.size() > 0) {
			...
			DZFDouble srje = null;
			DZFDouble srnum = null;
			DZFDouble fcje = null;
			DZFDouble fcnum = null;
			DZFDouble jcje = null;
			DZFDouble jcnum = null;
			String secondeKey = null;
			IcDetailVO skipvo = null;
			IcDetailVO ictempvo = null;

			for (String key : keySet) {
				account_id = key;
				// skipvo = new IcDetailVO();
				...
				skipvo = (IcDetailVO) qcrs.clone();
				// skipvo.setJcje(qcrs.getJcje());//计算器
				// skipvo.setJcsl(qcrs.getJcsl());
				result.put(key, qcrs);
				...
				for (String pkey : periodKeys) {
					if (count > 0) {
						skipvo.setSrsl(DZFDouble.ZERO_DBL);
						skipvo.setSrje(DZFDouble.ZERO_DBL);
						skipvo.setFcsl(DZFDouble.ZERO_DBL);
						skipvo.setFcje(DZFDouble.ZERO_DBL);
					...
					if (maps != null && !maps.isEmpty()) {
						...
						if (peList != null && peList.size() > 0) {
							// 排序
							Collections.sort(peList, new Comparator<IcDetailVO>() {
								@Override
								public int compare(IcDetailVO o1, IcDetailVO o2) {
									int i = o1.getDbilldate().compareTo(o2.getDbilldate());
									if (i == 0) {
										if (!StringUtil.isEmpty(o1.getDbillid())
												&& !StringUtil.isEmpty(o2.getDbillid())) {
							...
							for (IcDetailVO vo : peList) {
								srje = SafeCompute.add(skipvo.getSrje(), vo.getSrje());
								srnum = SafeCompute.add(skipvo.getSrsl(), vo.getSrsl());
								fcje = SafeCompute.add(skipvo.getFcje(), vo.getFcje());
								fcnum = SafeCompute.add(skipvo.getFcsl(), vo.getFcsl());

								jcje = SafeCompute.sub(SafeCompute.add(skipvo.getJcje(), vo.getSrje()), vo.getFcje());
								jcnum = SafeCompute.sub(SafeCompute.add(skipvo.getJcsl(), vo.getSrsl()), vo.getFcsl());

								skipvo.setSrsl(srnum);
								skipvo.setSrje(srje);
								skipvo.setFcsl(fcnum);
								skipvo.setFcje(fcje);
								if (StringUtil.isEmptyWithTrim(skipvo.getPk_sp())) {// 针对期初不存在进行特殊处理
									skipvo.setPk_sp(vo.getPk_sp());
									skipvo.setSpbm(vo.getSpbm());
									skipvo.setSpmc(vo.getSpmc());
									skipvo.setKm(vo.getKm());
									qcrs.setSpflcode(vo.getSpflcode());
									qcrs.setSpflid(vo.getSpflid());
									qcrs.setSpfl(vo.getSpfl());
									skipvo.setSpgg(vo.getSpgg());
									skipvo.setSpxh(vo.getSpxh());
									skipvo.setJldw(vo.getJldw());
									// skipvo.setDbilldate(paramvo.getBegindate1());

									qcrs.setPk_sp(vo.getPk_sp());
									qcrs.setSpbm(vo.getSpbm());
									qcrs.setSpmc(vo.getSpmc());
									qcrs.setKm(vo.getKm());
									qcrs.setSpflcode(vo.getSpflcode());
									qcrs.setSpflid(vo.getSpflid());
									qcrs.setSpfl(vo.getSpfl());
									qcrs.setSpgg(vo.getSpgg());
									qcrs.setSpxh(vo.getSpxh());
									qcrs.setJldw(vo.getJldw());
									qcrs.setDbilldate(paramvo.getBegindate1());

								}

								skipvo.setJcje(jcje);
								skipvo.setJcsl(jcnum);
								vo.setJcje(jcje);
								vo.setJcsl(jcnum);
								secondeKey = key + "," + vo.getPk_ictrade_h();
								// 如前边存在，需做加法运算
								if (result.containsKey(secondeKey)) {
									ictempvo = result.get(secondeKey);
									vo.setSrje(SafeCompute.add(vo.getSrje(), ictempvo.getSrje()));
									vo.setSrsl(SafeCompute.add(vo.getSrsl(), ictempvo.getSrsl()));
									vo.setFcje(SafeCompute.add(vo.getFcje(), ictempvo.getFcje()));
									vo.setFcsl(SafeCompute.add(vo.getFcsl(), ictempvo.getFcsl()));
								}
								result.put(secondeKey, vo);// 发生
							}
						} else {
							skipvo.setSrsl(DZFDouble.ZERO_DBL);
							skipvo.setSrje(DZFDouble.ZERO_DBL);
					...
					// 本期合计
					IcDetailVO bjvo = (IcDetailVO) skipvo.clone();
					bjvo.setZy("本期合计");
					bjvo.setBsyszy(DZFBoolean.TRUE);
					bjvo.setDbilldate(DateUtils.getPeriodEndDate(pkey));
					result.put(key + "," + pkey + ",bj", bjvo);
				}
	...
	private IcDetailVO calculateQC(String key, IcQcVO qcVo, List<IcDetailVO> periodBf, List<IcDetailVO> period,
		...
		if (qcVo != null) {// 期初
			...
			// skipvo.setPk_sp(qcVo.getPk_inventory());//存值后续用
