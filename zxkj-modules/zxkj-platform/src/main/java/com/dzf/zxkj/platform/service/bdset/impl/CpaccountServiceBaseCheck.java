...
public class CpaccountServiceBaseCheck {
    ...
    protected YntCpaccountVO getParentVOByID(String corpId, String subjectCode)
            throws BusinessException {
        ICpaccountService gl_cpacckmserv = (ICpaccountService) SpringUtils
                .getBean("gl_cpacckmserv");
        String accrule = gl_cpacckmserv.queryAccountRule(corpId);
        YntCpaccountVO vo = null;
        if (subjectCode.equals(
                DZfcommonTools.getFirstCode(subjectCode, accrule)))
            return vo;
        String qrysql = " SELECT * FROM YNT_CPACCOUNT WHERE PK_CORP = ? AND ACCOUNTCODE = ? AND NVL(DR,0)=0 ";
        SQLParameter SP = new SQLParameter();
        SP.addParam(corpId);
        String parentcode = DZfcommonTools.getParentCode(subjectCode,
                accrule);
