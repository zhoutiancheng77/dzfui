...
public class QueryMxNoIC {
    ...
    public QueryMxNoIC(SingleObjectBO singleObjectBO, IZxkjPlatformService zxkjPlatformService,
    ...
    public List<NumMnyDetailVO> querymx(String startDate, String enddate, String pk_corp, String user_id, QueryParamVO paranVo, String pk_bz, boolean bool, String xsfzhs, DZFDate begdate) throws DZFWarpException {
        ...
        for (YntCpaccountVO cpavo : cpalist) {
            ...
            if (!cpavo.getIsfzhs().equals("0000000000") && cpavo.getIsnum().booleanValue()) {
        ...
        //查询期初
        Map<String, QcYeVO> map2 = getQcMx(pk_corp, paranVo, pk_bz);

        List<NumMnyDetailVO> list = calcDetailVOs(jzdate, keylist, pk_corp, startDate, enddate, map1, map2, cpamap);
        ...
        list = getcqList(list, paranVo, pk_corp, user_id, cpamap);
        if ("Y".equals(xsfzhs)) {
            Map<String, String> kmbmmc = new HashMap<String, String>();
            Map<String, String> jldwmap = new HashMap<String, String>();
            Map<String, AuxiliaryAccountBVO> auaccountMap = zxkjPlatformService.queryAuxiliaryAccountBVOMap(pk_corp);
            List<String> fzkmbms = new ArrayList<String>();
            Map<String, FzhsqcVO> mapFzhsqc = getFzqcMx(pk_corp, paranVo, pk_bz, auaccountMap, cpamap);
            List<FzhsqcVO> fzqcs = new ArrayList<FzhsqcVO>(mapFzhsqc.values());
            for (FzhsqcVO vo : fzqcs) {
                String s[] = getCombStr(null, vo, auaccountMap, cpamap);
            ...
            for (NumMnyDetailVO vo : list1) {
                if (fzhskms.contains(vo.getPk_subject())) {
                    ...
                    String s[] = getCombStr(vo1, null, auaccountMap, cpamap);
                    if (!fzkmbms.contains(s[0])) {
                        fzkmbms.add(s[0]);
                        kmbmmc.put(s[0], s[1]);
...
//                }
    ...
    private void buildOthers(List<NumMnyDetailVO> list, NumMnyDetailVO cvo, List<NumMnyDetailVO> fzhslist,
                             String key, String kmbm, String jzdate, Map<String, FzhsqcVO> mapFzhsqc,
                             Map<String, String> kmbmmc, Map<String, String> jldwmap) {
    ...
    private NumMnyDetailVO buildfzhsqc(NumMnyDetailVO dvo, List<NumMnyDetailVO> fzhslist,
                                       Map<String, FzhsqcVO> mapFzhsqc, List<String> keys, String kmbm, String jzdate,
                                       Map<String, String> kmbmmc, Map<String, String> jldwmap) {
...
//					if(keys.get(0).substring(0,4).equals(jzdate.substring(0,4))){
                    ...
                    } else if(num1 != null && num1.doubleValue() != 0){
                        if("1".equals(vo.getDir())){
                            num = SafeCompute.sub(num, num1);
                        }else{
                            num = SafeCompute.add(num, num1);
                        }
...
//					}
    ...
    private String[] getCombStr(NumMnyDetailVO vo, FzhsqcVO vo1, Map<String, AuxiliaryAccountBVO> auaccountMap,
        ...
        if (vo != null) {
            ...
            if (!StringUtil.isEmpty(vo.getFzhsx6())) {
                ...
                name.append("_");
                name.append(auaccount.getName());
                if (!StringUtil.isEmpty(auaccount.getSpec())) {
                    name.append("(").append(auaccount.getSpec()).append(")");
                }
        ...
        if (vo1 != null) {
            //  期初开账新增  点确定后只保存科目
            if (StringUtil.isEmpty(vo1.getVcode())) {
                YntCpaccountVO tcpavo = cpamap.get(vo1.getPk_accsubj());
                if (tcpavo != null) {
                    code = new StringBuffer(tcpavo.getAccountcode());
                }
            } else if (vo1.getVcode().indexOf("_") > -1) {
                code = new StringBuffer(vo1.getVcode().substring(0, vo1.getVcode().indexOf("_")));
            } else {
                code = new StringBuffer(vo1.getVcode());
            }

            if (StringUtil.isEmpty(vo1.getVname())) {
                YntCpaccountVO tcpavo = cpamap.get(vo1.getPk_accsubj());
                if (tcpavo != null) {
                    name = new StringBuffer(tcpavo.getAccountname());
                }
            } else if (vo1.getVname().indexOf("_") > -1) {
                name = new StringBuffer(vo1.getVname().substring(0, vo1.getVname().indexOf("_")));
            } else {
                name = new StringBuffer(vo1.getVname());
            }
            AuxiliaryAccountBVO auaccount = null;
            if (!StringUtil.isEmpty(vo1.getFzhsx1())) {
                auaccount = auaccountMap.get(vo1.getFzhsx1());
                code.append("_");
                code.append(auaccount.getCode());
                name.append("_");
                name.append(auaccount.getName());
            }
            if (!StringUtil.isEmpty(vo1.getFzhsx2())) {
                auaccount = auaccountMap.get(vo1.getFzhsx2());
                code.append("_");
                code.append(auaccount.getCode());
                name.append("_");
                name.append(auaccount.getName());
            }
            if (!StringUtil.isEmpty(vo1.getFzhsx3())) {
                auaccount = auaccountMap.get(vo1.getFzhsx3());
                code.append("_");
                code.append(auaccount.getCode());
                name.append("_");
                name.append(auaccount.getName());
            }
            if (!StringUtil.isEmpty(vo1.getFzhsx4())) {
                auaccount = auaccountMap.get(vo1.getFzhsx4());
                code.append("_");
                code.append(auaccount.getCode());
                name.append("_");
                name.append(auaccount.getName());
            }
            if (!StringUtil.isEmpty(vo1.getFzhsx5())) {
                auaccount = auaccountMap.get(vo1.getFzhsx5());
                code.append("_");
                code.append(auaccount.getCode());
                name.append("_");
                name.append(auaccount.getName());
            ...
            if (!StringUtil.isEmpty(vo1.getFzhsx6())) {
                auaccount = auaccountMap.get(vo1.getFzhsx6());
                code.append("_");
                code.append(auaccount.getCode());
                name.append("_");
                name.append(auaccount.getName());
            ...
            if (!StringUtil.isEmpty(vo1.getFzhsx7())) {
                auaccount = auaccountMap.get(vo1.getFzhsx7());
                code.append("_");
                code.append(auaccount.getCode());
                name.append("_");
                name.append(auaccount.getName());
            }
            if (!StringUtil.isEmpty(vo1.getFzhsx8())) {
                auaccount = auaccountMap.get(vo1.getFzhsx8());
                code.append("_");
                code.append(auaccount.getCode());
                name.append("_");
                name.append(auaccount.getName());
            }
            if (!StringUtil.isEmpty(vo1.getFzhsx9())) {
                auaccount = auaccountMap.get(vo1.getFzhsx9());
                code.append("_");
                code.append(auaccount.getCode());
                name.append("_");
                name.append(auaccount.getName());
            }
            if (!StringUtil.isEmpty(vo1.getFzhsx10())) {
                auaccount = auaccountMap.get(vo1.getFzhsx10());
                code.append("_");
                code.append(auaccount.getCode());
                name.append("_");
                name.append(auaccount.getName());
    ...
    private List<NumMnyDetailVO> getcqList(List<NumMnyDetailVO> list, QueryParamVO paramvo, String pk_corp, String user_id, HashMap<String, YntCpaccountVO> cpamap) throws DZFWarpException {
...
//							v.setNnum(v.getNnumber().multiply(-1));
    ...
    //查询年期初数据
    public Map<String, QcYeVO> getQcMx(String pk_corp, QueryParamVO paranVo, String pk_bz) throws DZFWarpException {
        ...
        sf.append(" where  qc.pk_corp = ? and nvl(qc.dr,0) = 0 and  (nvl(ct.isnum,'N') = 'Y' or nvl(qc.monthqmnum,0) != 0) ");
        if (paranVo.getKms_first() != null && paranVo.getKms_first().length() > 0) {
            sf.append(" and qc.vcode >= ? ");
...
//		QcYeVO[] vos = (QcYeVO[])singleObjectBO.queryByCondition(QcYeVO.class, sf.toString(), sp);
        List<QcYeVO> list = (List<QcYeVO>) singleObjectBO.executeQuery(sf.toString(), sp, new BeanListProcessor(QcYeVO.class));
    ...
    //查询年期初数据
    public Map<String, FzhsqcVO> getFzqcMx(String pk_corp, QueryParamVO paranVo,
                                           String pk_bz, Map<String, AuxiliaryAccountBVO> auaccountMap,
                                           HashMap<String, YntCpaccountVO> cpamap) throws DZFWarpException {
        SQLParameter sp = new SQLParameter();
        sp.addParam(pk_corp);
        StringBuffer sf = new StringBuffer();
        ...
        sf.append(" join ynt_cpaccount ct ");
        sf.append(" on qc.pk_accsubj = ct.pk_corp_account ");
        sf.append(" where  qc.pk_corp = ? and nvl(qc.dr,0) = 0 and  (nvl(ct.isnum,'N') = 'Y' or nvl(qc.monthqmnum,0) != 0) and nvl(ct.isfzhs,'0000000000')!='0000000000' ");
        if (paranVo.getKms_first() != null && paranVo.getKms_first().length() > 0) {
        ...
        //这个考虑的时候，没有考虑外币情况
//			QcYeVO[] vos = (QcYeVO[])singleObjectBO.queryByCondition(QcYeVO.class, sf.toString(), sp);
        List<FzhsqcVO> list = (List<FzhsqcVO>) singleObjectBO.executeQuery(sf.toString(), sp, new BeanListProcessor(FzhsqcVO.class));
        FzhsqcVO[] vos = (FzhsqcVO[]) list.toArray(new FzhsqcVO[list.size()]);
        setVcodeValue(vos, auaccountMap, cpamap);
        Map<String, FzhsqcVO> map3 = hashlizeObject(vos);
        return map3;
    }

    private void setVcodeValue(FzhsqcVO[] vos,
                               Map<String, AuxiliaryAccountBVO> auaccountMap,
                               HashMap<String, YntCpaccountVO> cpamap) {
        if (vos == null || vos.length == 0
                || auaccountMap == null || auaccountMap.size() == 0) {
            return;
        }
        ...
        for (FzhsqcVO vo : vos) {
            code.setLength(0);
            name.setLength(0);
            if (StringUtil.isEmpty(vo.getVcode())) {
                key = vo.getPk_accsubj();
                cpavo = cpamap.get(key);
                if (cpavo != null) {
                    code.append(cpavo.getAccountcode());
                    name.append(cpavo.getAccountname());
                }

                //10个辅助项
                for (int i = 1; i <= 10; i++) {
                    fzhs = (String) vo.getAttributeValue("fzhsx" + i);
                    if (!StringUtil.isEmpty(fzhs)
                            && auaccountMap.containsKey(fzhs)) {
                        aabvo = auaccountMap.get(fzhs);
                        code.append("_").append(aabvo.getCode());
                        name.append("_").append(aabvo.getName());
                    }
                }
            }

            if (code.length() > 0) {
                vo.setVcode(code.toString());
                vo.setVname(name.toString());
            }
        }
    }
...
//				zlist1.get(i).setNprice(price);
                if (("本期合计".equals(zlist1.get(i).getZy()) && ReportUtil.bSysZy(zlist1.get(i)))
                        || ("本年累计".equals(zlist1.get(i).getZy())
                        && ReportUtil.bSysZy(zlist1.get(i)))
                ) {
                    zlist1.get(i).setNprice(price);
                }
            }
...
//				zlist1.get(i).setNdprice(price);
    ...
    public List<NumMnyDetailVO> calcDetailVOs(DZFDate jzdate, List<String> keylist, String pk_corp, String startDate, String enddate,
                                              Map<String, List<NumMnyDetailVO>> map1, Map<String, QcYeVO> map2, HashMap<String, YntCpaccountVO> cpamap) throws DZFWarpException {
        ...
        if (map1 != null && map1.size() > 0) {
            for (String key : keylist) {
                ...
                if (fsvos != null && fsvos.length > 0) {
                    NumMnyDetailVO c1 = (NumMnyDetailVO) qcvo.clone();
                    calcbq(list.toArray(new NumMnyDetailVO[list.size()]), zlist1, startDate, enddate, list, c1, qcyevos, pk_corp, cpamap);
                    //计算期末
...
//				}
        ...
        if (map2 != null && map2.size() > 0) {
            ...
            while (it.hasNext()) {
                ...
                qcvo.setKmbm(getKmbmByPk(key, pk_corp, cpamap));
                ...
                if (!keylis.contains(getKmbmByPk(key, pk_corp, cpamap))) {
                    qcyevo.put(qcvo.getKmbm(), qcvo);
                }
            }
        }
        if (map2 != null && map2.size() > 0) {
            ...
            while (it.hasNext()) {
                ...
                QcYeVO vo = map2.get(key);
                List<NumMnyDetailVO> zlist1 = new ArrayList<NumMnyDetailVO>();
                NumMnyDetailVO zvo = build(jzdate, vo, startDate, pk_corp, qcyevo, zlist, cpamap);
                zlist1.add(zvo);
                ...
                for (NumMnyDetailVO allvo : alllist) {
                    NumMnyDetailVO nvoclone = null;
                    boolean bool = judgeisparent(allvo.getKmbm(), zvo.getKmbm());
                    if (bool && allvo.getPzh() != null && allvo.getPzh().length() > 0) {
                        nvoclone = (NumMnyDetailVO) allvo.clone();
                    ...
                    if (allvo.getPzh() != null && allvo.getPzh().length() > 0 && (bool || allvo.getKmbm().equals(zvo.getKmbm()))) {
                        if (nvoclone == null) {
                            nvoclone = (NumMnyDetailVO) allvo.clone();
                        }
...
//					}
                ...
                if ((fsvos != null && fsvos.size() > 0) || (bnljvos != null && bnljvos.size() > 0)) {
                    ...
                    calcbq(fsvos.toArray(new NumMnyDetailVO[fsvos.size()]), zlist1, startDate, enddate, bnljvos, c1, qcyevos, pk_corp, cpamap);
                    //计算期末
                    calcQM(zlist1, cpamap);
                    //计算单价
                    calcprice(zlist1);
                    ...
                    for (String tempKey : keys) {
                        ...
                        for (QcYeVO qc : qcyevos) {
                            if (qcyear == qc.getVyear().intValue() && qc.getMonthqmnum().doubleValue() != 0 && c2.getKmbm().equals(getKmbmByPk(qc.getPk_accsubj(), pk_corp, cpamap))) {
...
//							z6 = qc.getYeardffse();
    ...
    public NumMnyDetailVO build(DZFDate jzdate, QcYeVO banvo, String startDate, String pk_corp, Map<String, NumMnyDetailVO> qcyevomap, List<NumMnyDetailVO> zlist1, Map<String, YntCpaccountVO> map) throws DZFWarpException {
        ...
        NumMnyDetailVO mvo = queryINfo(banvo.getPk_accsubj(), pk_corp, map);
        cvo.setPk_subject(mvo.getPk_subject());
        cvo.setJldw(mvo.getJldw());
        cvo.setKmbm(mvo.getKmbm());
        cvo.setKmmc(mvo.getKmmc());
        cvo.setSpmc(mvo.getSpmc());
        cvo.setDir(mvo.getDir());
        cvo.setQj(startqj);
