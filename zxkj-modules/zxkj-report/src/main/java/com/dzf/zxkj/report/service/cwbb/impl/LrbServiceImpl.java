...
public class LrbServiceImpl implements ILrbService {
    ...
    @Autowired
    private ILrbQuarterlyService lrbQuarterlyService;

    @Override
    public LrbVO[] getLRBVOs(LrbQueryVO queryVO, CorpVO corpVO) throws Exception {
        return getLRBVOsConXm(queryVO, null, corpVO);
    }

    @Override
    public LrbVO[] getLRBVOsConXm(LrbQueryVO queryVO, List<String> xmid, CorpVO corpVO) throws Exception {
        /** 来源利润表 */
        FsYeQueryVO fsYeQueryVO = new FsYeQueryVO();
        fsYeQueryVO.setRptsource("lrb");
        fsYeQueryVO.setFirstlevelkms(rptSetService.queryLrbKmsFromDaima(corpVO, xmid));
        int year = new DZFDate(fsYeQueryVO.getQjq() + "-01").getYear();
        fsYeQueryVO.setPk_corp(corpVO.getPk_corp());
        fsYeQueryVO.setQjq(year + "-01");
        if (!StringUtil.isEmpty(queryVO.getXmlbid()) && !StringUtil.isEmpty(queryVO.getXmmcid())) {
            fsYeQueryVO.setSfzxm(DZFBoolean.TRUE);
        }
        Object[] obj = fsYeService.getFsJyeVOs1(fsYeQueryVO);
        ...
        LrbVO[] lrbvos = getLrbVos(queryVO, corpVO, mp, map, queryVO.getXmmcid());
    ...
    @Override
    public LrbVO[] getLrbVosFromFs(LrbQueryVO queryVO, Map<String, YntCpaccountVO> mp, CorpVO corpVO, FseJyeVO[] fvos) throws Exception {
        Map<String, FseJyeVO> map = new HashMap<String, FseJyeVO>();
        int len = fvos == null ? 0 : fvos.length;
        for (int i = 0; i < len; i++) {
            if (fvos[i] != null) {
        ...
        return getLrbVos(queryVO, corpVO, mp, map, "");
    }

    @Override
    public LrbVO[] getLRBVOsByPeriod(LrbQueryVO queryVO, CorpVO corpVO) throws Exception {
        /** 来源利润表 */
        FsYeQueryVO fsYeQueryVO = new FsYeQueryVO();
        fsYeQueryVO.setRptsource("lrb");
        fsYeQueryVO.setFirstlevelkms(rptSetService.queryLrbKmsFromDaima(corpVO, null));
        if (!StringUtil.isEmpty(queryVO.getXmlbid()) && !StringUtil.isEmpty(queryVO.getXmmcid())) {
            fsYeQueryVO.setSfzxm(DZFBoolean.TRUE);
        }
        Object[] obj = fsYeService.getFsJyeVOs1(fsYeQueryVO);
        ...
        return getLrbVosByPeriod(queryVO, corpVO, mp, map, queryVO.getXmmcid());
    }
     ...
     * @return
     */
    private LrbVO[] getLrbVosByPeriod(LrbQueryVO vo, CorpVO corpVO, Map<String, YntCpaccountVO> mp,
                                      Map<String, FseJyeVO> map, String xmmcid) throws Exception {

        Integer corpschema = zxkjPlatformService.getAccountSchema(corpVO.getPk_corp());

        if (corpschema == DzfUtil.SEVENSCHEMA.intValue()) {
            /** 2007会计准则 */
            return getLRB2007VOsByPeriod(map, mp, corpVO, xmmcid);
        } else if (corpschema == DzfUtil.THIRTEENSCHEMA.intValue()) {
            /** 2013会计准则 */
            return getLRB2013VOsByPeriod(map, mp, corpVO, xmmcid);
        } else {
            throw new RuntimeException("该制度暂不支持利润表,敬请期待!");
        }
    }
     ...
     */
    private LrbVO[] getLRB2007VOsByPeriod(Map<String, FseJyeVO> map, Map<String, YntCpaccountVO> mp, CorpVO corpVO,
                                          String xmmcid) throws Exception, Exception {
        ...
        String queryAccountRule = zxkjPlatformService.queryAccountRule(corpVO.getPk_corp());
     ...
     * @throws BusinessException
     */
    private LrbVO[] getLRB2013VOsByPeriod(Map<String, FseJyeVO> map, Map<String, YntCpaccountVO> mp, CorpVO corpVO,
                                          String xmmcid) throws Exception, Exception {
        LrbVO vo1 = new LrbVO();
        vo1.setXm("一、营业收入");
    ...
    @Override
    public Map<String, DZFDouble> getYearLRBVOs(String year, Object[] objs, CorpVO corpVO) throws Exception {
        Object[] obj = fsYeService.getYearFsJyeVOs(year, corpVO.getPk_corp(), objs, "lrb");
    ...
    @Override
    public Map<String, List<LrbVO>> getYearLrbMap(String year, CorpVO corpVO, String xmmcid, Object[] objs, DZFBoolean ishasjz) throws Exception {
        Object[] obj = fsYeService.getYearFsJyeVOs(year, corpVO.getPk_corp(), xmmcid, objs, "lrb", ishasjz);
        ...
        for (String str : yearset) {
            ...
            if (corpschema == DzfUtil.SEVENSCHEMA.intValue()) {
                /** 2007会计准则 */
                lrbvos = getLRB2007VOs(map, mp, str.substring(0, 7), corpVO, xmmcid);
            } else if (corpschema == DzfUtil.THIRTEENSCHEMA.intValue()) {
                /** 2013会计准则 */
                lrbvos = getLRB2013VOs(map, mp, str.substring(0, 7), corpVO, xmmcid);
            } else if (corpschema == DzfUtil.COMPANYACCOUNTSYSTEM.intValue()) {//企业会计制度
                OtherSystemForLrb lrb_qykj = new OtherSystemForLrb(rptSetService);
                lrbvos = lrb_qykj.getCompanyVos(map, mp, str.substring(0, 7), corpVO, xmmcid, "00000100000000Ig4yfE0005");
            }
...
//			}
    ...
    @Override
    public List<LrbVO[]> getBetweenLrbMap(DZFDate begdate, DZFDate enddate, CorpVO corpVO, String xmmcid, Object[] objs, DZFBoolean ishasjz) throws Exception {
        if (begdate == null || enddate == null || corpVO == null || StringUtil.isEmpty(corpVO.getPk_corp())) {
            throw new RuntimeException("查询参数不能为空!");
        }

        List<String> periods = ReportUtil.getPeriods(begdate, enddate);
        Object[] obj = fsYeService.getEveryPeriodFsJyeVOs(begdate, enddate, corpVO.getPk_corp(), objs, "lrb", ishasjz);
    ...
    @Override
    public LrbVO[] getLrbDataForCwBs(String qj, CorpVO corpVO, String qjlx) throws Exception {
        LrbQueryVO paramVO = new LrbQueryVO();
        LrbVO[] lrbvos = null;
...
//			lrbvos = gl_rep_lrbserv.getLRBVOs(paramVO);
            lrbvos = getLRBVOs(paramVO, corpVO);
            /** 如果是一般企业则取上年数据 */
            Integer corpschema = zxkjPlatformService.getAccountSchema(corpVO.getPk_corp());
            /** 07科目体系(一般企业) */
            if (corpschema == DzfUtil.SEVENSCHEMA.intValue()) {
                ...
                /** 公司建账日期在查询日期后时 */
                if (DateUtils.getPeriod(corpVO.getBegindate()).compareTo(bef_period) <= 0) {
                    ...
                    bef_lrbvos = getLRBVOs(paramVO, corpVO);
            ...
            LrbQuarterlyQueryVO lrbQuarterlyQueryVO = new LrbQuarterlyQueryVO();
            BeanUtils.copyProperties(paramVO, lrbQuarterlyQueryVO);
            LrbquarterlyVO[] lrbjb = lrbQuarterlyService.getLRBquarterlyVOs(lrbQuarterlyQueryVO, corpVO);
    ...
    private LrbVO[] convertLrb(LrbquarterlyVO[] listVo, String qj) {
        if (listVo == null || listVo.length == 0) {
            throw new RuntimeException("利润表季报数据为空");
        }
     ...
     * @throws BusinessException
     */
    private LrbVO[] getLRB2007VOs(Map<String, FseJyeVO> map, Map<String, YntCpaccountVO> mp, String qjz, CorpVO corpVO,
                                  String xmmcid) throws Exception {
        String queryAccountRule = zxkjPlatformService.queryAccountRule(corpVO.getPk_corp());
        LrbVO vo1 = new LrbVO();
        vo1.setXm("一、营业收入");
     ...
     * @throws BusinessException
     */
    private LrbVO[] getLRB2013VOs(Map<String, FseJyeVO> map, Map<String, YntCpaccountVO> mp, String rq, CorpVO corpVO,
                                  String xmmcid) throws Exception {
        LrbVO vo1 = new LrbVO();
        vo1.setXm("一、营业收入");
    ...
    @Override
    public LrbVO[] getLrbVos(LrbQueryVO queryVO, CorpVO corpVO, Map<String, YntCpaccountVO> mp, Map<String, FseJyeVO> map, String xmmcid) throws Exception {
        Integer corpschema = zxkjPlatformService.getAccountSchema(corpVO.getPk_corp());
        LrbVO[] lrbvos = null;
        /** 2007会计准则(一般) */
        if (corpschema == DzfUtil.SEVENSCHEMA.intValue()) {
            lrbvos = getLRB2007VOs(map, mp, queryVO.getQjz(), corpVO, xmmcid);
        } else if (corpschema == DzfUtil.THIRTEENSCHEMA.intValue()) {/** 2013会计准则(小企业) */
            lrbvos = getLRB2013VOs(map, mp, queryVO.getQjz(), corpVO, xmmcid);
        } else if (corpschema == DzfUtil.COMPANYACCOUNTSYSTEM.intValue()) {/** 企业会计制度 */
            OtherSystemForLrb lrb_qykj = new OtherSystemForLrb(rptSetService);
            lrbvos = lrb_qykj.getCompanyVos(map, mp, queryVO.getQjz(), corpVO, xmmcid, "00000100000000Ig4yfE0005");
        } else {
            throw new RuntimeException("该制度暂不支持利润表,敬请期待!");
        }
