...
public class LrbQuarterlyServiceImpl implements ILrbQuarterlyService {
    ...
    @Override
    public LrbquarterlyVO[] getLRBquarterlyVOs(LrbQuarterlyQueryVO queryVO, CorpVO corpVO) throws Exception {
        int year = new DZFDate(queryVO.getQjq() + "-01").getYear();
        int lastyear = year - 1;
        String pk_corp = corpVO.getPk_corp();
        DZFBoolean ishasjz = queryVO.getIshasjz();
        queryVO.setPk_corp(pk_corp);
        queryVO.setRptsource("lrb");
        DZFDate corpdate = corpVO.getBegindate();
        Object[] lastyearobj = null;
        if (lastyear < corpdate.getYear()) {
        } else {
            lastyearobj = fsYeService.getYearFsJyeVOsLrbquarter(lastyear + "", pk_corp, corpdate, ishasjz, queryVO.getRptsource());
        }
        Object[] obj = fsYeService.getYearFsJyeVOsLrbquarter(year + "", pk_corp, corpdate, ishasjz, queryVO.getRptsource());
        ...
        for (int i = 0; i < sortedMonthmap.size(); i++) {
            ...
            if (key.compareTo(queryVO.getQjz()) <= 0) {
    ...
    private LrbquarterlyVO[] getCompanyAccountVos(LrbquarterlyVO[] lrbquartervos, Map<String, FseJyeVO> map,
        ...
        LrbRptSetVo[] setvos = null; //todo
//                (LrbRptSetVo[]) singleObjectBO.queryByCondition(LrbRptSetVo.class,
//                "nvl(dr,0)=0 and pk_trade_accountschema = ? ", sp);
    ...
    private LrbquarterlyVO[] getLRB2013VOs(LrbquarterlyVO[] lrbquartervos, Map<String, FseJyeVO> map,
                                           Map<String, YntCpaccountVO> mp, String rq, int year, String pk_corp)
            throws Exception {
        LrbVO[] lrbvos = lrbService.getLrbVos(new LrbQueryVO(), pk_corp, new HashMap<String, YntCpaccountVO>(), null, "");
    ...
    @Override
    public Map<String, LrbquarterlyVO[]> getLRBquarterlyVOs(LrbQuarterlyQueryVO vo, Object[] objs, CorpVO corpVO) throws Exception {

        if (objs == null || objs.length == 0) {
            throw new RuntimeException("明细账数据为空!");
        }
        ...
        for (String str : periods) {
            ...
            if ("03".equals(month) || "06".equals(month) || "09".equals(month) || "12".equals(month)) {
                ...
                Object[] fsobjs = fsYeService.getEveryPeriodFsJyeVOs(startdate, enddate, pk_corp, objs, "lrb", vo.getIshasjz());
    ...
    public Map<String, DZFDouble> getYearLRBquarterlyVOs(String year, String pk_corp) throws Exception {
        Object[] obj = fsYeService.getYearFsJyeVOs(year,pk_corp,null,"lrb");
