...
public class ExcelExportGuangXiHander extends ExcelExportHander implements OneWorkBookKj2007Excel, MoreWorkBookKj2013Excel {

    @Override
    public Workbook createWorkBookZcfzKj2013(Map<String, ZcFzBVO> vOMap, Map<String, String> taxaxVoMap) throws Exception {
        Resource resource = ResourceUtil.get(ExportTemplateEnum.GUANGXI, ResourceUtil.ResourceEnum.KJ2013ZCFZ);
        Workbook workbook = WorkbookFactory.create(resource.getInputStream());
        Sheet sheet = workbook.getSheetAt(0);
        handleZcfzbSheet(sheet, taxaxVoMap, vOMap, 6, new Integer[]{0,2,3,4,6,7},new String[]{"qmye1","ncye1","qmye2","ncye2"});
        return workbook;
    }

    @Override
    public Workbook createWorkBookLrbKj2013(Map<String, LrbVO> vOMap, Map<String, String> taxaxVoMap) throws Exception {
        Resource resource = ResourceUtil.get(ExportTemplateEnum.GUANGXI, ResourceUtil.ResourceEnum.KJ2013LR);
        Workbook workbook = WorkbookFactory.create(resource.getInputStream());
        Sheet sheet = workbook.getSheetAt(0);
        handleLrbSheet(sheet, taxaxVoMap, vOMap, 5, new Integer[]{0,3,6}, new String[]{"bnljje","byje"});
        return workbook;
    }
...
//        Resource resource = ResourceUtil.get(ExportTemplateEnum.GUANGXI, ResourceUtil.ResourceEnum.KJ2013ALL);
//        Workbook workbook = WorkbookFactory.create(resource.getInputStream());
//        //资产负债表
//        Sheet sheet = workbook.getSheetAt(0);
//        handleZcfzbSheet(sheet, zcfzTaxVoMap, zcFzBVOMap, 8, new Integer[]{2, 3, 4, 6, 7, 8},new String[]{"qmye1","ncye1","qmye2","ncye2"});
