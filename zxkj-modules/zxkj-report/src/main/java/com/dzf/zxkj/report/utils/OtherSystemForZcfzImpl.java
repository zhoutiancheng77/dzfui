...
public class OtherSystemForZcfzImpl {
    ...
    public OtherSystemForZcfzImpl(IRptSetService rptSetService, IZxkjPlatformService zxkjPlatformService) {
        this.rptSetService = rptSetService;
        this.zxkjPlatformService = zxkjPlatformService;
    }

    public ZcFzBVO[] getCompanyZCFZBVOs(Map<String, FseJyeVO> map, String[] hasyes, Map<String, YntCpaccountVO> mapc, String hy, String pk_corp) throws Exception {

        if (StringUtil.isEmpty(hy)) {
            throw new RuntimeException("行业为空!");
        }
        // 查询设置数据
        ZcfzRptSetVo[] zcfzRptSetVos = rptSetService.queryZcfzRptVOs(hy);

        if (zcfzRptSetVos == null || zcfzRptSetVos.length == 0) {
        ...
        for (ZcfzRptSetVo setvo : zcfzRptSetVos) {
    ...
    private ZcFzBVO getZCFZBVO(Map<String, FseJyeVO> map, ZcFzBVO vo, boolean is1, String newrule, String... kms) throws Exception {
    ...
    private List<FseJyeVO> getData1(Map<String, FseJyeVO> map, String kmtemp, Map<String, YntCpaccountVO> mapc, String newrule) throws Exception {
    ...
    private List<FseJyeVO> getData(Map<String, FseJyeVO> map, String kmtemp, String newrule) throws Exception {
        //考虑科目升级
        String km = zxkjPlatformService.getNewRuleCode(kmtemp, DZFConstant.ACCOUNTCODERULE, newrule);
        List<FseJyeVO> list = new ArrayList<FseJyeVO>();
        for (FseJyeVO fsejyevo : map.values()) {
