...
public class Qy07XjlReportService {
	...
	public XjllbVO[] getXJLLB2007VOs1(Map<Float, XjllbVO> map, String period,
									  String pk_trade_accountschema, Map<String, FseJyeVO> fsmap, CorpVO cpvo, YntCpaccountVO[] cpavos) throws Exception {
		...
		// 如果查询月份小于建账日期
		DZFDate corpdate = new DZFDate(DateUtils.getPeriod(cpvo.getBegindate()) + "-01");
		...
		// "N");//年初
		// }
		getSanKmQcmny(fsmap, corpdate, vo34, period, cpvo, cpavos, DZFBoolean.TRUE);
		...
		if (currdate.before(corpdate)) {
			...
			getSanKmQmMny(fsmap, voLiu, DZFBoolean.TRUE,cpvo, cpavos);
		}
	...
	private void getSanKmQmMny(Map<String, FseJyeVO> fsmap, XjllbVO voLiu,DZFBoolean is07,CorpVO corpVO,YntCpaccountVO[] yntCpaccountVO) {
		Set<String> kmcode =  KmschemaCash.getCashSubjectCode1(yntCpaccountVO, corpVO.getCorptype(),DZFBoolean.TRUE);
	...
	private void getSanKmQcmny(Map<String, FseJyeVO> fsmap ,DZFDate corpdate ,
			XjllbVO  vo , String period , CorpVO corpVO, YntCpaccountVO[] yntCpaccountVOS, DZFBoolean is07) throws Exception{
		Set<String> kmcode =  KmschemaCash.getCashSubjectCode1(yntCpaccountVOS, corpVO.getCorptype(),DZFBoolean.TRUE);
		DZFDouble sqje = DZFDouble.ZERO_DBL;
		DZFDouble bqje = DZFDouble.ZERO_DBL;
		FseJyeVO fvo1  =null;
		for(String str:kmcode){
