...
public class QueryMxKC {
	...
	public QueryMxKC(SingleObjectBO singleObjectBO, IZxkjPlatformService zxkjPlatformService,
...
//			YntCpaccountVO tcpavo = null;
			for(FzhsqcVO vo : fzqcs){
				String s[] = getCombStr(null,vo, auaccountMap, inventoryMap, cpamap);
				if(!fzkmbms.contains(s[0])){
					fzkmbms.add(s[0]);
					kmbmmc.put(s[0], s[1]);
//					if(StringUtil.isEmpty(s[2]) && !StringUtil.isEmpty(vo.getPk_accsubj())){
//						tcpavo = cpamap.get(vo.getPk_accsubj());
//						if(tcpavo != null){
//							s[2] = tcpavo.getMeasurename();
//						}
//					}
			...
			for(NumMnyDetailVO vo : list1){
				//预防这种情况发生，比如/1405/开始没有启用辅助核算，但启用数量金额，所以存在一部分余额。后来又启用辅助核算。
				//因此这部分统计去掉
				if(fzhskms.contains(vo.getPk_subject())
						&& (!StringUtil.isEmpty(vo.getFzhsx1()) ||
								!StringUtil.isEmpty(vo.getFzhsx2()) ||
								!StringUtil.isEmpty(vo.getFzhsx3()) ||
								!StringUtil.isEmpty(vo.getFzhsx4()) ||
								!StringUtil.isEmpty(vo.getFzhsx5()) ||
								!StringUtil.isEmpty(vo.getFzhsx6()) ||
								!StringUtil.isEmpty(vo.getFzhsx7()) ||
								!StringUtil.isEmpty(vo.getFzhsx8()) ||
								!StringUtil.isEmpty(vo.getFzhsx9()) ||
								!StringUtil.isEmpty(vo.getFzhsx10()))){
					NumMnyDetailVO vo1 = (NumMnyDetailVO) vo.clone();
					String s[] = getCombStr(vo1,null, auaccountMap, inventoryMap, cpamap);
					if(!fzkmbms.contains(s[0])){
						fzkmbms.add(s[0]);
						kmbmmc.put(s[0], s[1]);
						jldwmap.put(s[0], s[2]);
...
//					if(keys.get(0).substring(0,4).equals(jzdate.substring(0,4))){
								...
								DZFDouble num1 = getDouNullAsZero(vo.getNnumber());
								DZFDouble jf =getDouNullAsZero(vo.getJfmny());
								DZFDouble df =getDouNullAsZero(vo.getDfmny());
								if("1".equals(cvo.getDir())){
									if(jf != null && jf.doubleValue() > 0){
										num = SafeCompute.sub(num, num1);
										mny = SafeCompute.sub(mny, jf);
									}else if(df != null && df.doubleValue() < 0){
										num = SafeCompute.sub(num, num1.multiply(-1));
										mny = SafeCompute.sub(mny, df.multiply(-1));
									}else if(jf != null && jf.doubleValue() < 0){
										num = SafeCompute.add(num, num1.multiply(-1));
										mny = SafeCompute.add(mny, jf.multiply(-1));
									}else if(df != null && df.doubleValue() > 0){
...
//				InventoryVO inventory = inventoryMap.get(vo.getFzhsx5());
//				if(inventory != null){
//					code.append("_");
...
//				str[2] = auaccount.getUnit();
	...
	private DZFDouble getDouNullAsZero(DZFDouble d){
		if(d == null){
			return DZFDouble.ZERO_DBL;
		}
		return d;
...
//				sf.append(" and nvl(th.ishasjz,'N')  ");
	...
	public Map<String,QcYeVO> getQcMx(String pk_corp,QueryParamVO paranVo,String pk_bz)throws DZFWarpException {
		...
		sf.append(" where  qc.pk_corp = ? and nvl(qc.dr,0) = 0 and nvl(ct.dr,0) = 0 and  (nvl(ct.isnum,'N') = 'Y' or nvl(qc.monthqmnum,0) != 0) ");
...
//				}
				if(map2.get(key) == null || (fsvos != null && fsvos.length > 0))
					zlist.addAll(zlist1);
		}
		}
		...
		if(map2 != null && map2.size() > 0){
			...
			while(it.hasNext()){
				...
				QcYeVO vo = map2.get(key);
				List<NumMnyDetailVO> zlist1 = new ArrayList<NumMnyDetailVO>();
				NumMnyDetailVO zvo = build(jzdate,vo,startDate,pk_corp,qcyevo,zlist,cpamap,cpamap);
				zlist1.add(zvo);
...
//							z6 = qc.getYeardffse();
	...
	public NumMnyDetailVO queryINfo(String pk_accsubj,String pk_corp)throws DZFWarpException {
		StringBuffer sf = new StringBuffer();
		sf.append(" select t.fullname,t.pk_corp_account,t.accountcode,t.measurename,t.direction  from ynt_cpaccount t ");
...
//		vo1.setOpdate(opdate);
	...
	private QcYeVO calcnumqc(NumMnyDetailVO[] qcvos,QcYeVO banvo,String dir)throws DZFWarpException {
		if(banvo == null )
			return null;
		...
		if(qcvos != null && qcvos.length > 0){
			for(NumMnyDetailVO v : qcvos){
				DZFDouble num =getDouNullAsZero(v.getNnumber()) ;
				DZFDouble jf =getDouNullAsZero(v.getJfmny());
				DZFDouble df =getDouNullAsZero(v.getDfmny());
				
				if("1".equals(dir)){
					if(jf != null && jf.doubleValue() > 0){
						bc = SafeCompute.sub(bc, num);
						ac = SafeCompute.sub(ac, jf);
					}else if(df != null && df.doubleValue() < 0){
						bc = SafeCompute.sub(bc, num.multiply(-1));
						ac = SafeCompute.sub(ac, df.multiply(-1));
					}else if(jf != null && jf.doubleValue() < 0){
						bc = SafeCompute.add(bc, num.multiply(-1));
						ac = SafeCompute.add(ac, jf.multiply(-1));
					}else if(df != null && df.doubleValue() > 0){
	...
	public NumMnyDetailVO bulidQC(DZFDate jzdate,NumMnyDetailVO dvo,String startDate,NumMnyDetailVO[] qcvos,QcYeVO banvo,HashMap<String, YntCpaccountVO> cpamap)throws DZFWarpException {
		...
		if((qcvos != null && qcvos.length > 0)||(banvo!=null)){
			...
			calcnumqc(qcvos,banvo,cvo.getDir());
			DZFDouble mny = banvo.getThismonthqc();
			DZFDouble num = banvo.getMonthqmnum();
...
//				mny = SafeCompute.multiply(mny, new DZFDouble(-1));
//			}
//			banvo.setThismonthqc(mny);
//			banvo.setMonthqmnum(num);
			cvo.setNynum(num);
			cvo.setNymny(mny);
			if(mny != null && num!= null && num.doubleValue() != 0){
				DZFDouble price = SafeCompute.div(mny, num);
