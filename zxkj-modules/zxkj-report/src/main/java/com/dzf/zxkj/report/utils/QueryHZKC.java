...
public class QueryHZKC {
	...
	public QueryHZKC(SingleObjectBO singleObjectBO,IZxkjPlatformService zxkjPlatformService,
...
//		whereSql.append(")");
	...
	private List<NumMnyGlVO> queryDetailVOs(DZFDate jzDate,
		...
		sf.append(" join ynt_cpaccount c on b.pk_accsubj = c.pk_corp_account ");
		sf.append(" left join ynt_tzpz_h h on h.pk_tzpz_h = b.pk_tzpz_h ");
		sf.append(" where b.pk_corp = ? and nvl(b.dr,0) = 0 and c.pk_corp = ? ");
		sf.append("  and nvl(c.dr,0) = 0 and (nvl(c.isnum, 'N') ='Y' or b.nnumber > 0 or b.nnumber < 0 ) and nvl(h.dr,0) = 0 and h.period >= ? and h.period <= ? ");
		if (pk_bz != null && pk_bz.length() > 0) {
