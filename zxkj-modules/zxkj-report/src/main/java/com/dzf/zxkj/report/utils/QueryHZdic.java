...
public class QueryHZdic {
	...
	public QueryHZdic(SingleObjectBO singleObjectBO,
	...
	public List<NumMnyGlVO> buildData(QueryCondictionVO paramVo)
		...
		Map<String, FzhsqcVO> qcMap = getQcMx(paramVo);
	...
	private void addParents(Map<String, NumMnyGlVO> result,
		...
		for (String leafKey : leaves) {
			YntCpaccountVO ctvo = mp.get(leafKey);
			if(ctvo == null)
				continue;
			if(ctvo.getIsleaf() == null 
					|| !ctvo.getIsleaf().booleanValue())
				continue;
	...
	private NumMnyGlVO calculateAll(String key, FzhsqcVO qcVo,
		...
		if (!AuxiliaryConstant.ACCOUNT_FZHS_DEFAULT.equals(account.getIsfzhs())) {
			copyFzPks(vo, qcVo, yearBf, periodBf, period, periodEndYear);
		}
	 ...
	 * @param src
	 */
	private void copyFzPks(SuperVO dest, SuperVO... src) {
		if (dest == null) {
			return;
		}
		for (SuperVO srcVo : src) {
			if (srcVo != null) {
				for (int i = 1; i <= 10; i++) {
					String attr = "fzhsx" + i;
					String id = (String) srcVo.getAttributeValue(attr);
	...
	private Map<String, FzhsqcVO> getQcMx(QueryCondictionVO paramVo)
		...
		sf.append(" select * from ynt_qcye qc ");
		
		whereSql.append(" join ynt_cpaccount ct ");
		whereSql.append(" on qc.pk_accsubj = ct.pk_corp_account ");
		whereSql.append(" where qc.pk_corp = ? and ct.pk_corp = ? and nvl(qc.dr,0)=0 and (nvl(ct.isnum,'N') = 'Y' or nvl(qc.monthqmnum,0)!=0) and isleaf = 'Y' ");
...
//		whereSql.append(" (select pk_corp_account from ynt_cpaccount cp where cp.pk_corp = ? and nvl(cp.dr, 0) = 0 and isnum = 'Y' and isleaf = 'Y' ");
		...
		if (!StringUtil.isEmptyWithTrim(paramVo.getKms_first())) {
			whereSql.append(" and ct.accountcode >= ? ");
...
//		whereSql.append(")");
		if (pk_bz != null && pk_bz.length() > 0) {
			whereSql.append(" and qc.pk_currency = ?");
			sp.addParam(pk_bz);
		}
		sf.append(whereSql);
		...
		if (isfzhs) {
			//查询辅助核算期初
			String sql = " select * from ynt_fzhsqc qc " + whereSql.toString();
	...
	private List<NumMnyGlVO> queryDetailVOs(DZFDate jzDate,
		...
		sf.append(" where b.pk_corp = ? and t.pk_corp = ?  and (nvl(t.isnum, 'N') = 'Y' or b.nnumber > 0 or b.nnumber < 0 ) and nvl(t.dr,0)=0 ");
		sf.append(" and nvl(b.dr,0) = 0  and h.period >= ? and h.period <= ? ");//and ( b.nnumber > 0 or b.nnumber < 0 )
		if (pk_bz != null && pk_bz.length() > 0) {
...
//				v.setBqjfnum(num);
	...
	private List<NumMnyGlVO> filterAndSetValue(Map<String, NumMnyGlVO> result,
		...
		for (String rs_key : result.keySet()) {
			...
			if (account.getAccountlevel() >= cjq
					&& account.getAccountlevel() <= cjz && !checkEmpty(paramvo,vo)) {
				...
				if (isFzhs && rs_key.length() > 24) {
					String[] fzStr = getFzCombStr(keyArray, auaccountMap);
	...
	//检查数据是否为空
	private boolean checkEmpty (QueryCondictionVO paramvo,NumMnyGlVO vo) {
		return (paramvo.getXswyewfs()!=null && paramvo.getXswyewfs().booleanValue() )
				&&  (vo.getQcmny() == null || vo.getQcmny().doubleValue() == 0)
